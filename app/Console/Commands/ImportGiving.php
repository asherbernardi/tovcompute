...
class ImportGiving extends Command
{
    protected $signature = 'giving:import
                            {--csv= : Path to the 990 CSV file (defaults to data_sources/ directory)}
                            {--background : Run in background and return immediately}';

    protected $description = 'Import charity giving data from a 990 Combined DataMart CSV file';
    ...
    public function handle(): int
    {
        ...
        $cmd = implode(' ', [
            ...
            escapeshellarg(base_path('scripts/import_giving.py')),
        ...
        if ($this->option('background')) {
            $logFile = storage_path('logs/giving_import.log');
            exec("nohup {$cmd} > " . escapeshellarg($logFile) . " 2>&1 &");
            $this->info("Giving import started in the background.");
