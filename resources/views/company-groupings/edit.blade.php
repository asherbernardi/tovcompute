<!-- resources/views/company-groupings/edit.blade.php -->

@extends('layout.app')

@section('content')
    <div class="container mx-auto px-4 sm:px-6 lg:px-8">
        <h1 class="text-2xl font-bold mb-4">Edit Grouping: {{ $companyGrouping->name }}</h1>
        ...
        <!-- Edit Grouping Name -->
        <form method="POST" action="{{ route('company-groupings.update', $companyGrouping) }}" class="max-w-md mx-auto mb-8">
            @csrf
            @method('PUT')

            <div class="mb-4">
                <label for="name" class="block text-sm font-medium text-gray-700">Grouping Name</label>
                <input type="text" name="name" id="name" value="{{ old('name', $companyGrouping->name) }}"
        ...
        <div class="mb-8">
            <h2 class="text-xl font-semibold mb-4">Add Companies to Grouping</h2>
                        ...
                        @foreach ($companies as $company)
                            <tr class="company-row" data-name="{{ strtolower($company->name) }}" data-ticker="{{ strtolower($company->ticker ?? '') }}">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    @if ($companyGrouping->companies->contains($company->id))
                                        <span class="text-gray-500">Already Associated</span>
                                    @else
                                        <form action="{{ route('company-groupings.add-companies', $companyGrouping) }}" method="POST" class="inline-block">
                                            @csrf
        ...
        <!-- Current Companies -->
        <div>
            <h2 class="text-xl font-semibold mb-4">Companies Currently in Grouping</h2>
            <!-- Search Input -->
            <div class="mb-4 max-w-md mx-auto">
            ...
            <div class="overflow-x-auto overflow-y-scroll" style="height:300px;">
                <table class="min-w-full bg-white border border-gray-200" id="current-companies-table">
                    ...
                    <tbody class="divide-y divide-gray-200">
                        @forelse ($companyGrouping->companies as $company)
                            <tr class="company-row" data-name="{{ strtolower($company->name) }}" data-ticker="{{ strtolower($company->ticker ?? '') }}">
                                <td class="px-6 py-4 whitespace-nowrap">{{ $company->name }}</td>
                                <td class="px-6 py-4 whitespace-nowrap">{{ $company->ticker ?? 'N/A' }}</td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <form action="{{ route('company-groupings.remove-company', [$companyGrouping, $company->id]) }}" method="POST" class="inline-block" onsubmit="return confirm('Remove {{ $company->name }} from this grouping?');">
                                        @csrf
                                        @method('DELETE')
                        ...
                        @empty
                            <tr>
                                <td colspan="3" class="px-6 py-4 text-center text-gray-500">No companies in this grouping.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
